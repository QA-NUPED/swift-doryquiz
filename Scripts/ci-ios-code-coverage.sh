...
# Nome do arquivo de saída LCOV
lcov_file="${$RESULT_JSON}.lcov"

# Crie um arquivo LCOV vazio
echo "TN:" > "$lcov_file"

cat "$RESULT_JSON" | jq -r '.[] | "SF:\(.sourceFile)" as $sf | .lines | keys[] as $line | "DA:\($line),\(.[$line])"' >> "$lcov_file"

# Adicione o marcador de final de registro
echo "end_of_record" >> "$lcov_file"

echo "Conversão concluída. O arquivo LCOV foi salvo como: $lcov_file"
